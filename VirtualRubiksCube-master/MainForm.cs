...
namespace VirtualRubiksCube
{
    public partial class MainForm : Form
    {
        ...
        private Queue<string> solutionQueue = new Queue<string>();
        private RubiksCube rubiksCube;
        private RubiksCubeController controller;
        private RubiksCubeRenderer renderer;
        private string currentCubeState;  // Lưu trạng thái màu sắc của Rubik
        ...
        private void enterColorToolStripMenuItem_Click(object sender, EventArgs e) // xu li khi chuc nang nhap mau dc kich hoat
        {
            if (!renderer.IsRunning || controller.CurrentRotationInfo.IsRotating)
                return;

            EnterColorIForm colorInputForm = new EnterColorIForm();
            colorInputForm.Owner = this; // Đặt Owner là MainForm
            colorInputForm.ShowDialog();
        }
        ...
        private void ExecuteMove(string move)
        {
            switch (move)
            {
                case "U": uButton_Click(uButton, EventArgs.Empty); break;
                case "U'": uPrimeButton_Click(uPrimeButton, EventArgs.Empty); break;

                case "D": dButton_Click(dButton, EventArgs.Empty); break;
                case "D'": dPrimeButton_Click(dPrimeButton, EventArgs.Empty); break;

                case "L": lButton_Click(lButton, EventArgs.Empty); break;
                case "L'": lPrimeButton_Click(lPrimeButton, EventArgs.Empty); break;

                case "R": rButton_Click(rButton, EventArgs.Empty); break;
                case "R'": rPrimeButton_Click(rPrimeButton, EventArgs.Empty); break;

                case "F": fButton_Click(fButton, EventArgs.Empty); break;
                case "F'": fPrimeButton_Click(fPrimeButton, EventArgs.Empty); break;

                case "B": bButton_Click(bButton, EventArgs.Empty); break;
                case "B'": bPrimeButton_Click(bPrimeButton, EventArgs.Empty); break;
        ...
        private async void MainForm_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    string move = solutionQueue.Dequeue();
                    ExecuteMove(move);
                }
                return;
            }

            if (e.KeyCode == Keys.O) // Xoay toàn bộ chuỗi một cách liên tục
            {
        ...
        private void uPrimeButton_Click(object sender, EventArgs e)
        {
            ...
            if (rotateRadioButton.Checked)//xoay ngay 
            {
                controller.StartRotation(newMove);
            }
            else// thêm bước xoay vào danh sách chờ 
            {
                controller.MoveQueue.Add(newMove);
                moveQueueBindingSource.ResetBindings(false);
                moveQueueListBox.SelectedIndex = moveQueueListBox.Items.Count - 1;
        ...
        private void setQueueToSolutionButton_Click(object sender, EventArgs e)
        {
            ...
            // Lấy trạng thái của Rubik
            Demo demo = new Demo();
            string state = controller.stateTo54sign();
            List<string> solution = demo.Solve(state);

            // Kiểm tra nếu có lời giải hợp lệ
            if (solution.Count > 2)
            {
                // Xóa hàng đợi cũ và nạp bước giải mới
                solutionQueue.Clear();

                // Ghép hai chuỗi 
                string combinedSolution = (solution[0].Trim() + " " + solution[1].Trim()).Trim();
                string[] moves = combinedSolution.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);

                // Sửa chuỗi: thay "X2" thành "X X", "F2" thành "F F",...
                List<string> processedMoves = new List<string>();
                foreach (string move in moves)
                {
                    if (!string.IsNullOrWhiteSpace(move))
                    {
                        if (move.EndsWith("2"))
                        {
                            string baseMove = move.Substring(0, move.Length - 1); // Bỏ số 2
                            processedMoves.Add(baseMove);
                            processedMoves.Add(baseMove);
                        }
                        else
                        {
                ...
                // Đưa vào queue để thực hiện xoay
                foreach (string move in processedMoves)
                {
                    solutionQueue.Enqueue(move);
                }
                // Hiển thị lại chuỗi đã sửa
                label3.Text = "Phase 1: " + solution[0] + "Phase 2: " + solution[1] + "Max steps: " + solution[2];
                label3.Font = new Font("Arial", 15, FontStyle.Bold); 
                label3.ForeColor = Color.Purple; 
            }
        }

        public void ApplyReverseSolution(string reversedSolution)
        {
            if (!renderer.IsRunning || controller.CurrentRotationInfo.IsRotating)
                return;
            //ResetCube();
            ...
            string[] moves = reversedSolution.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (string move in moves)
            {
                if (!string.IsNullOrWhiteSpace(move))
                {
                    ...
                    {
                        string baseMove = move.Substring(0, move.Length - 1); // Bỏ số 2
                        processedMoves.Add(ParseMove(baseMove));
                        processedMoves.Add(ParseMove(baseMove)); // Thêm hai lần
                    }
                    else
                    {
                        processedMoves.Add(ParseMove(move));
                    }
                }
            }

            // Gọi StartRotation từ RubiksCubeController
            controller.Scramble(processedMoves);
        }


        private VirtualRubiksCube.Move ParseMove(string move)
        {
            VirtualRubiksCube.Move.RotationType rotationType = VirtualRubiksCube.Move.RotationType.Clockwise;
            ...
            // Xác định layer dựa trên ký tự đầu tiên
            RubiksCube.Layer layer;
            switch (move)
            {
                case "U": layer = RubiksCube.Layer.Up; break;
                case "D": layer = RubiksCube.Layer.Down; break;
                case "F": layer = RubiksCube.Layer.Front; break;
                case "B": layer = RubiksCube.Layer.Back; break;
                case "L": layer = RubiksCube.Layer.Left; break;
                case "R": layer = RubiksCube.Layer.Right; break;
                case "M": layer = RubiksCube.Layer.MiddleX; break;
                case "E": layer = RubiksCube.Layer.MiddleY; break;
                case "S": layer = RubiksCube.Layer.MiddleZ; break;
                default:
                    throw new ArgumentException($"Không xác định được bước di chuyển: '{move}'");
            }
            return new Move(layer, rotationType);
        }
        #endregion
    }
}
